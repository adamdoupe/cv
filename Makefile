DOC = adam_doupe_cv.tex
PDFLATEX = pdflatex -interaction=nonstopmode -file-line-error
BIBTEX = bibtex
RERUN = "(There were undefined (references|citations)|Rerun to get (cross-references|the bars) right|Etaremune labels have changed)"
...
%.pdf: %.tex *.tex $(BIB_FILES) $(IMG_OBJS) $(ALL_TEX_FILES)
	$(eval DOC_BASE := $(shell basename $@ .pdf))
	rm -f $(DOC_BASE).bbl
	${PDFLATEX} $<
...
loop:
	while true; do inotifywait -e modify -r . --exclude "output/*"; make all; done
